...
namespace PLAYERTWO.PlatformerProject
{
	...
	public class Player : Entity<Player>
	{
		...
		{
			if ((!onWater ||
				 !states.IsCurrentOfType(typeof(SwimPlayerState))) &&
				 !health.isEmpty)
			{
				Throw();
				onWater = true;
		...
		public virtual void Accelerate(Vector3 direction)
		{
			...
			var running = inputs.GetRun() && stats.current.canRun && (stats.current.canRunOnAir || isGrounded);
			var topSpeed = running ? stats.current.runningTopSpeed : stats.current.topSpeed;

			Accelerate(direction, turningDrag, finalAcceleration, topSpeed);

			if (inputs.GetRunUp() && isGrounded)
			{
		...
		public virtual void WallDrag(Collider other)
		{
			...
			var detectingGround = SphereCast(-transform.up, minGroundDistance, out var groundHit);
			var wallAngle = Vector3.Angle(transform.up, hit.normal);
			var groundAngle = Vector3.Angle(transform.up, groundHit.normal);

			if (!detectingWall || (detectingGround && groundAngle < controller.slopeLimit) ||
				detectingLedge || wallAngle < stats.current.minWallAngleToDrag)
				return;
